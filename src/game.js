...
export default () => {
	...
	document
		.querySelectorAll(".opponent-square")
		.forEach((square) => square.addEventListener("click", makeTurn));
	...
	function makeTurn(event) {
		...
		document
			.querySelectorAll(".opponent-square")
			.forEach((square) => square.removeEventListener("click", makeTurn));
